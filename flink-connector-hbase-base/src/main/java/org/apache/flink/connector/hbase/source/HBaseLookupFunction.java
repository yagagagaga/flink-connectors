...
public class HBaseLookupFunction extends TableFunction<Row> {
	...
	private transient Cache<Get, Row> cache;
	...
	public void eval(Object rowKey) throws IOException {
		Get get = readHelper.createGet(rowKey);
		if (cache != null) {
			Row cachedRow = cache.getIfPresent(get);
			if (cachedRow != null) {
				collect(cachedRow);
				return;
			}
		...
		if (!result.isEmpty()) {
			// parse and collect
			Row row = readHelper.parseToRow(result, rowKey);
			collect(row);
			cache.put(get, row);
		}
	}
